...
func MQRead(messageChan chan<- []byte, qName string, comTag string) {
	...
	zap.L().Info(qName + " queue init successful, waiting for messages")
	zap.L().Info("[*] Waiting for logs. To exit press CTRL+C", zap.String("queue name", qName))
	<-forever
}
